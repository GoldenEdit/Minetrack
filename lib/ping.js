      ...
      serverRegistration.dnsResolver.resolve((host, port, remainingTimeout) => {
        ...
        server.ping(remainingTimeout, version, (err, res) => {
          if (err) {
            ...
            const payload = {
              players: {
                online: capPlayerCount(serverRegistration.data.ip, parseInt(res.players.online), serverRegistration.data.maxPlayerCount)
              },
    ...
    case 'PE':
      minecraftBedrockPing(serverRegistration.data.ip, serverRegistration.data.port || 19132, (err, res) => {
        if (err) {
          ...
          callback(null, {
            players: {
              online: capPlayerCount(serverRegistration.data.ip, parseInt(res.currentPlayers), serverRegistration.data.maxPlayerCount)
            }
          })
        }
      }, timeout)
...
function capPlayerCount (host, playerCount, serverMaxPlayerCount) {
  if (serverMaxPlayerCount && playerCount > serverMaxPlayerCount) {
    logger.log('warn', '%s returned a spoofed player count of %d (max %d), ignoring ping', host, playerCount, serverMaxPlayerCount)
    return null
  }
